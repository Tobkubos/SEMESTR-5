    ...
    if (query.exec("SELECT * FROM wypozyczenia"))
    {
        while (query.next())
        {
            ...
            if (osobaQuery.exec() && osobaQuery.next()) {
                ...
                osobaImieNazwisko = "brak imie lub nazwisko";
            }
            ...
            if (ksiazkaQuery.exec() && ksiazkaQuery.next()) {
                ...
                ksiazkaTytulAutor = "brak tytul lub autor";
            }

            QString radioButtonName = QString::number(id) + ". " + osobaImieNazwisko + " wypozyczyl/a: " + ksiazkaTytulAutor;
...
void MainWindow::CreateWypozyczenie(QSqlQuery &query, int osobaId, int ksiazkaId){
    ...
    if (query.exec()) {
        ...
        cout << "Bladd: Nie udalo sie dodac ksiazki!" << endl;
    }
