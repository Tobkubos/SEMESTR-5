...
void myRender::paintEvent(QPaintEvent * /* _event */)
{
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            double cNormalized = v * 1. / iters * 1.;

            //kolory
            double redValue = exp(-w1 * pow(abs(cNormalized - c1), p1));
            double greenValue = exp(-w2 * pow(abs(cNormalized - c2), p2));
            double blueValue = exp(-w3 * pow(abs(cNormalized - c3), p3));

            int red = redValue * 255;
            int green = greenValue * 255;
            int blue = blueValue * 255;
